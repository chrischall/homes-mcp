...
| `homes_get_session_context` / `homes_register_session` / `homes_set_active_session` | List / register / switch account *labels* for your own bookkeeping. Label only: requests always use whichever account the fetchproxy browser tab is signed into. |
